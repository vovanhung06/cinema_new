...
export default function Checkout() {
  ...
  // ─── Guard 1: chưa login → redirect về login ─────────────────────────────
  useEffect(() => {
    if (!loading && !user) {
  ...
  const [plan] = useState(() => {
    if (location.state?.plan) {
      localStorage.setItem('checkout_plan', JSON.stringify({
        ...location.state.plan,
        userId: user?.id,
      }));
  ...
  // ─── Lưu session vào localStorage (chỉ khi tạo mới) ──────────────────────
  useEffect(() => {
    if (!user || !randomCode || isSuccess || isExpired) return;
    const saved = localStorage.getItem('checkout_session');
  ...
  }, [randomCode, user, isSuccess, isExpired]);

  // ─── Đăng ký session với backend ─────────────────────────────────────────
  const registerSession = useCallback(async (code) => {
    if (!user) return;
    if (user.is_vip) return;
    try {
      await axios.post(
        `${API_BASE_URL}/vip/session`,
        { randomCode: code, amount: priceNumber },
        getAuthHeaders()
      );
    } catch (e) {
  ...
  // ─── Đồng hồ đếm ngược + xử lý hết hạn ──────────────────────────────────
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      try {
        const res = await axios.get(
          `${API_BASE_URL}/vip/session/${randomCode}`,
    ...
    return (
      <div className="min-h-screen bg-surface flex flex-col items-center justify-center text-white relative overflow-hidden">
        ...
        <motion.div
          ...
          className="relative z-10 flex flex-col items-center justify-center p-12 glass-dark rounded-[3rem] border border-white/5 text-center shadow-2xl"
        >
          ...
          <p className="text-on-surface-variant text-sm font-medium mb-10 max-w-sm leading-relaxed">
            Vui lòng quay lại danh sách các gói VIP để lựa chọn một gói phù hợp.
          </p>
          <button
            onClick={() => navigate('/vip')}
            className="btn-primary px-8 py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] flex items-center gap-3"
          >
    ...
    <div className="min-h-screen bg-surface pt-32 pb-24 px-6 text-white relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto space-y-12 relative z-10">
        <header className="flex items-center gap-6">
          <button
            onClick={() => navigate(-1)}
            className="w-14 h-14 flex items-center justify-center glass-dark rounded-3xl border border-white/10 hover:border-primary/50 hover:bg-primary/10 transition-all group shadow-xl"
          >
                ...
                {isExpired ? (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="btn-primary px-10 py-4 rounded-2xl font-black uppercase text-[11px] tracking-[0.3em] flex items-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-[0_15px_30px_rgba(229,9,20,0.3)]"
                    >
