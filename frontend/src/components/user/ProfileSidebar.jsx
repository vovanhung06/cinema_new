    ...
    <>
      ...
      <div className="lg:hidden fixed top-20 left-0 w-full z-40 bg-surface/80 backdrop-blur-xl border-b border-white/5 px-4">
        <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar py-4 scroll-smooth">
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                className={`flex-none flex items-center gap-2 px-5 py-3 rounded-2xl transition-all relative ${isActive ? 'text-white' : 'text-on-surface-variant hover:text-white'}`}
              >
                {isActive && (
                  <motion.div
          ...
          <button
            onClick={logout}
            className="flex-none flex items-center gap-2 px-5 py-3 rounded-2xl text-red-500 hover:bg-red-500/10 transition-all"
          >
            <LogOut className="w-4 h-4" />
            <span className="text-[10px] font-black uppercase tracking-widest whitespace-nowrap">Đăng xuất</span>
          </button>
        </div>
      </div>
    </>
