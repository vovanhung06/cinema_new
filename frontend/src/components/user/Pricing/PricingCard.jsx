...
export default function PricingCard({
  ...
  const handleClick = () => {
    ...
    // Lấy trạng thái VIP trực tiếp từ user (nguồn chính xác nhất)
    // thay vì chỉ dựa vào prop isVip có thể bị stale
    if (isVip || user?.is_vip) return;
  ...
  // Ưu tiên trạng thái live từ user context, prop isVip là fallback
  const activeVip = isVip || !!user?.is_vip;
    ...
    >
      ...
      <div className="relative z-10 space-y-6">
        <div className="space-y-2">
          <h4 className={`font-black text-3xl italic uppercase tracking-tighter ${activeVip ? 'text-green-400' : 'text-white'}`}>
            {title}
          </h4>
        ...
        <div className="py-8 border-y border-white/5 space-y-1">
          <div className="flex items-baseline gap-1">
            <span className={`text-5xl font-black italic tracking-tighter ${activeVip ? 'text-green-400' : 'text-white'}`}>
              {price}
            </span>
          ...
          {features.map((feature, index) => (
            <li key={index} className="flex items-start gap-4 group/item">
              <div className={cn(
                ...
                activeVip ? "bg-green-500/10" : "bg-red-500/10 group-hover/item:bg-red-500/20"
              )}>
                {activeVip ? (
      ...
      <Zap className={cn(
        ...
        activeVip ? "text-green-500/5" : "text-white/5 opacity-20 group-hover:scale-125 group-hover:text-red-500/10"
      )} />
    </motion.div>
  );
}